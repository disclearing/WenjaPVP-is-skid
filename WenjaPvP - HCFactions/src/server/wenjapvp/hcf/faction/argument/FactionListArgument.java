...
public class FactionListArgument extends CommandArgument
{
	...
	private void showList(final int pageNumber, final String label, final CommandSender sender)
	{
		...
		for (Map.Entry<PlayerFaction, Integer> entry : sortedMap)
		{
			...
			ComponentBuilder builder = new ComponentBuilder("  " + index + ". ").color(net.md_5.bungee.api.ChatColor.AQUA);
		...
		sender.sendMessage(ChatColor.YELLOW + ChatColor.BOLD.toString() + " FACTION LIST " + ChatColor.AQUA + "-" + ChatColor.GRAY + " (Page " + pageNumber + '/' + maxPages + ')');
		...
		sender.sendMessage(ChatColor.GOLD + ChatColor.BOLD.toString() + " * " + ChatColor.YELLOW + "You are currently on " + ChatColor.GRAY + "Page " + pageNumber + '/' + maxPages + ChatColor.GOLD + '.');
		sender.sendMessage(ChatColor.GOLD + ChatColor.BOLD.toString() + " * " + ChatColor.YELLOW + "To view other pages, use " + ChatColor.GRAY + '/' + label + ' ' + this.getName() + " <page#>" + ChatColor.GOLD + '.');
