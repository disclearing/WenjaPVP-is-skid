...
public class FactionLivesDepositArgument extends CommandArgument
{
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		...
		int currentLives = this.plugin.getDeathbanManager().getPublicLives(player.getUniqueId());
		if (currentLives <= 0)
		{
			sender.sendMessage(ChatColor.RED + "You do not have enough lives to deposit.");
			return true;
		}
		if (currentLives - amount < 0)
		{
			sender.sendMessage(ChatColor.RED + "Your lives after this transaction would be less than 0.");
			return true;
		}
		FactionMember factionMember = playerFaction.getMember(player);

		playerFaction.setLives(playerFaction.getLives() + amount);
		this.plugin.getDeathbanManager().takePublicLives(player.getUniqueId(), amount);
