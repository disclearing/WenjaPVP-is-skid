...
public class EconomyCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if(args.length < 2 || !hasStaffPermission) {
            sender.sendMessage(ChatColor.GOLD + (sender.equals(target) ? "Your balance" : ("Balance of " + target.getName())) + " is " + ChatColor.WHITE + EconomyManager.ECONOMY_SYMBOL + balance + ChatColor.YELLOW + '.');
            return true;
        }
