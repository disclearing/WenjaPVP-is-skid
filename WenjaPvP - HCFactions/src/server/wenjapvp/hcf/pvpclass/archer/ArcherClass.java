package server.wenjapvp.hcf.pvpclass.archer;
        ...
        implements Listener
{
    ...
    public static  final TObjectLongMap<UUID> archerJumpCooldowns = new TObjectLongHashMap();
            ...
            if ((source instanceof Player))
            {
                ...
                if ((this.plugin.getTimerManager().archerTimer.getRemaining((Player)entity) == 0L) || (this.plugin.getTimerManager().archerTimer.getRemaining((Player)entity) < TimeUnit.SECONDS.toMillis(5L)))
                {
                    ...
                    for (Player player : Bukkit.getServer().getOnlinePlayers()) {
                        this.plugin.getScoreboardHandler().getPlayerBoard(player.getUniqueId()).addUpdates(Bukkit.getServer().getOnlinePlayers());
                    }
