...
public class CoreListener implements Listener
{
	...
	// final Faction toFactionAt =
	// this.plugin.getFactionManager().getFactionAt(event.getTo());
	// if (toFactionAt.isSafezone() &&
	// !this.plugin.getFactionManager().getFactionAt(event.getFrom()).isSafezone())
	// {
	// final Player player = event.getPlayer();
	// player.sendMessage(ChatColor.RED + "You may not Enderpearl into
	// safe-zones, used Enderpearl has been refunded.");
	...
	/*
	 * @EventHandler(ignoreCancelled = true, priority = EventPriority.HIGH)
	 * public void onPlayerPortal(final PlayerPortalEvent event) { if
	 * (event.getCause() == PlayerTeleportEvent.TeleportCause.NETHER_PORTAL) {
	 * final Location from = event.getFrom(); final Location to = event.getTo();
	 * final Player player = event.getPlayer(); final Faction fromFac =
	 * this.plugin.getFactionManager().getFactionAt(from); if
	 * (fromFac.isSafezone()) {
	 * event.setTo(to.getWorld().getSpawnLocation().add(0.5, 0.0, 0.5));
	 * event.useTravelAgent(false); player.sendMessage(ChatColor.YELLOW +
	 ...
	 * (!travelAgent.getCanCreatePortal()) { return; } final Location
	 * foundPortal = travelAgent.findPortal(to); if (foundPortal != null) {
	 * return; } final Faction factionAt =
	 * this.plugin.getFactionManager().getFactionAt(to); if (factionAt
	 * instanceof ClaimableFaction) { final Faction playerFaction =
	 * this.plugin.getFactionManager().getPlayerFaction(player); if
	 * (playerFaction != null && playerFaction.equals(factionAt)) { return; }
	 * player.sendMessage(ChatColor.YELLOW +
	 * "Portal would have created portal in territory of " +
	 * factionAt.getDisplayName(playerFaction) + ChatColor.YELLOW + '.');
	 * event.setCancelled(true); } } } }
	 */
