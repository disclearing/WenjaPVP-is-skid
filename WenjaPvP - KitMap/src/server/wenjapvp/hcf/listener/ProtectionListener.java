...
public class ProtectionListener implements Listener {
        ...
        if (action == Action.PHYSICAL) { // Prevent players from trampling on crops or pressure plates, etc.
            ...
            if (canBuild) {
                ...
                if (itemType != null && ITEM_BLOCK_INTERACTABLES.containsKey(itemType) && ITEM_BLOCK_INTERACTABLES.get(itemType).contains(event.getClickedBlock().getType())) {
                    if (block.getType() != Material.WORKBENCH || !event.getPlayer().hasPermission("openchestsilent") || !plugin.getFactionManager().getFactionAt(block).isSafezone()) {
                        canBuild = false;
                    }
                } else {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (HCF.getPlugin().getFactionManager().getFactionAt(event.getPlayer().getLocation()) == HCF.getPlugin().getFactionManager().getFaction("GlowstoneMountain")){
